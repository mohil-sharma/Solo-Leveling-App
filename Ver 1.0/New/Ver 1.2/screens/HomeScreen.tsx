...
const HunterDashboard = () => {
  // ... [Keep all the state declarations and functions as they are]
      ...
      >
        <View style={styles.container}>
          {showQuestComplete && (
            ...
            >
              ...
              >
                ...
                <Text style={styles.questRewardText}>+{questCompleteAnimation ? 100 : 0} XP</Text>
              </LinearGradient>
            </Animated.View>
          )}
          ...
          <ScrollView style={styles.scrollView}>
            {/* Keep the rest of the JSX structure as is */}
          </ScrollView>
        </View>
      </ImageBackground>
    </View>
...
export default HunterDashboard;
  ...
  const [shadows, setShadows] = useState<Shadow[]>([
    {
      ...
      loyalty: 50,
      specialAbilities: {
        "Shield Wall": { unlocked: false, requirement: 5 },
        "Iron Defense": { unlocked: false, requirement: 10 }
      }
    },
    {
      id: 2,
  ...
  const enterDungeon = (dungeon: Dungeon) => {
    if (hunterStats.dungeonKeys <= 0) {
      alert('You need dungeon keys to enter!');
      return;
    }

    // Check if player meets rank requirement
    if (getRankValue(hunterStats.rank) < getRankValue(dungeon.level)) {
      alert(`You need to be at least rank ${dungeon.level} to enter this dungeon!`);
      return;
    }
  ...
  const checkRankUp = async (newXP: number) => {
    ...
    if (currentXP >= hunterStats.xpToNextLevel) {
      ...
      if (currentRankIndex < ranks.length - 1) {
        ...
        celebrateRankUp();
        // Need to define playRankUpSound function or import it
        // For example:
        // import { playRankUpSound } from '../utils/sounds';
        // Or define the function:
        // const playRankUpSound = () => {
        //   // Play rank up sound logic
        // };
  ...
  const QuestCard: React.FC<{quest: Quest}> = ({ quest }) => {
    const addCustomQuest = (questData: Quest) => {
      const newQuest = {
        ...
        ...questData,
        completed: false,
        reward: generateReward(hunterStats.rank)
      };
