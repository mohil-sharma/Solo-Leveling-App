...
const HunterDashboard = () => {
  ...
  const summonShadow = (shadow: Shadow) => {
    ...
    Animated.parallel([
      ...
      Animated.sequence([
        Animated.spring(dungeonAnimation, {
          toValue: 1,
  ...
  const [showLevelUpEffects, setShowLevelUpEffects] = useState(false);
  ...
  const animateRewardBox = () => {
    ...
    particleSystem.forEach((particle: any) => particle.animation.setValue(0));
    ...
    Animated.parallel([
      ...
      // Particle effects
      ...particleSystem.map((particle: any) =>
        Animated.sequence([
  ...
  const checkRankUp = async (newXP: number) => {
    ...
    if (currentXP >= hunterStats.xpToNextLevel) {
      ...
      if (currentRankIndex < ranks.length - 1) {
        ...
        setHunterStats((prev: HunterStats) => ({
          ...prev,
        ...
        const newShadow = shadows.find((s: Shadow) => s.rank === newRank);
        if (newShadow) {
          setShadows((prev: Shadow[]) =>
            prev.map((shadow: Shadow) =>
  ...
  const generateRankQuests = (rank: string) => {
    ...
    const rankQuests: Record<string, Quest[]> = {
      D: [
        { id: 4, name: 'Iron Body Training', xp: 200, completed: false, target: 40, type: 'strength', 
          reward: generateQuestReward('D'), requirement: 'Unlocks Iron Shadow Soldier' },
      ],
      C: [
        { id: 5, name: 'Heavy Lifting', xp: 250, completed: false, target: 50, type: 'strength',
          reward: generateQuestReward('C'), requirement: 'Unlocks Tank Shadow Soldier' },
      ],
      B: [
        { id: 6, name: 'Assassin Sprint', xp: 300, completed: false, target: 5, type: 'cardio',
          reward: generateQuestReward('B'), requirement: 'Unlocks Beru Shadow Soldier' },
      ],
      A: [
        { id: 7, name: 'Knight\'s Challenge', xp: 400, completed: false, target: 100, type: 'mixed',
          reward: generateQuestReward('A'), requirement: 'Unlocks Igris Shadow Soldier' },
      ],
    };
  ...
  const startSummoningAnimation = (shadow: Shadow) => {
    ...
    const particleSystem = Array.from({ length: particleCount }, () => ({
      animation: new Animated.Value(0),
      scale: 0.5 + Math.random() * 1.5,
      angle: Math.random() * Math.PI * 2,
    ...
    if (typeof showSystemNotification === 'function') {
      ...
      console.error('showSystemNotification function is not defined');
    }

    if (typeof getRankTension === 'function' && typeof getRankAuraDuration === 'function') {
      Animated.parallel([
      ...
      console.error('getRankTension or getRankAuraDuration function is not defined');
    }
  ...
  const renderShadowModal = () => {
    if (!selectedShadow) {
      console.error('No selected shadow to display in modal');
      return null;
    }
