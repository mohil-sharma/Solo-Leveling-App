...
const HunterDashboard = () => {
  ...
  const [hunterStats, setHunterStats] = useState({
  ...
  });  const [shadows, setShadows] = useState([
    {
      ...
      loyalty: 0,
      specialAbilities: {
        "Iron Wall": { unlocked: false, requirement: 5 },
        "Shield Bash": { unlocked: false, requirement: 10 },
        "Protective Aura": { unlocked: false, requirement: 15 }
      },
  ...
  const startDungeonChallenge = (dungeon) => {
    // Deduct key
    setHunterStats(prev => ({
      ...prev,
  ...
  const [availableDungeons, setAvailableDungeons] = useState([    {
    ...
    const generateQuestReward = (rank) => {
      const rewards = {
        'E': [
      ...
      return rewards[rank][Math.floor(Math.random() * rewards[rank].length)];
    };
    ...
    const baseQuests = [
      { 
        ...
        type: 'strength',
        reward: generateQuestReward(rank),
      },
      { 
        id: 2, 
        name: 'Shadow Sprint', 
  ...
  const QuestCard = ({ quest }) => {  const addCustomQuest = (questData) => {
    const newQuest = {
      id: Date.now(),
      ...questData,
      completed: false,
  ...
  const generateSideQuests = () => {
    ...
    ).start();      const currentDate = new Date().toDateString();
        ...
        >
          ...
          >
            <View style={styles.modalContainer}>              <View style={styles.modalContent}>
                ...
                {/* Removed duplicate and incorrect JSX structure */},
...
const styles = StyleSheet.create({
  ...
  progressText: {
    ...
    overflow: 'hidden',
    backgroundColor: 'rgba(11, 22, 34, 0.95)', // Almost solid dark blue
    borderWidth: 1,
    borderColor: 'rgba(0, 148, 255, 0.3)',
    shadowColor: "#0094FF",
    shadowOffset: {
